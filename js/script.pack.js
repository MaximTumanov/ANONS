...
var i18n = {
	user_not_found: {
		ru: "Только для <a href='"+host+"users/'>авторизированных</a> пользователей"
	},
	event_not_found: {
	...
	must_confirm: {
		ru: 'Нам необходимо Ваше согласие с публичной офертой'
	}
}
...
$(function () {
	...
	if(typeof(use_payu) != "undefined" && use_payu === true){

		var generate_payu_button = function(id_event, count, date, submit){
			var submit = submit || false;
			$.ajax({
				type: "POST",
				url: "/events/ticket",
				async: false,
				data: {id_event: id_event, count: count, date: date},
				success: function(msg){
					$('body').append(msg);
					if (submit) {
						$('#payu_form').submit();
		...
		$('.jqTransformCheckbox').click(function(e){
			e.preventDefault();
			$(this).toggleClass('jqTransformCheckedBox');
			$('#offer-trigger').poshytip('hide');
		})

		$('#payu_count').filter_input({regex:'[0-9]'});

		$('#payu_button').click(function(e){
			e.preventDefault();
			var count = parseFloat($('#payu_count').val());

			$('#offer-trigger').poshytip({
				content: i18n.must_confirm[lang],
			...
			if (user.u.is_logged) {
				if ($('.jqTransformCheckedBox').length > 0){
					generate_payu_button(id_event, count, date, true);
				} else {
					//dialog.show(i18n.must_confirm[lang], 'dialog_error');
					$('#offer-trigger').poshytip('show');
				}
			} else {
				dialog.show(i18n.user_not_found[lang], 'dialog_error');
			}
		...
		$('#payu_count').bind('change keyup', function(){
			var el = $(this)
				,	count = parseFloat(el.val())
				,	price = parseFloat($('#payu_price').val())
				,	commission = parseFloat($('#payu_commission').val())
				,	total_wrapp = $('#e_total')
				,	commission_wrapp = $('#e_commission')
				, total;

			if(count > parseFloat(el.attr('max'))){ count = el.attr('max'); el.val(count); }
			if(count < 0 || isNaN(count)){ count = 1; el.val(count); }
			total = count * price * (1 + (commission/100))

			commission_wrapp.text((count * price * (commission/100)).toFixed(2));
			total_wrapp.text(total.toFixed(2));
		});

	}

	if(typeof(check_ipn) != "undefined" && check_ipn === true){
		var check_count = 0
			,	preloader = $('.proccessing_loader')
			,	proccessing_text = $('.proccessing_loader_text');
		var check = function(){
			$.ajax({
				url: host + 'proccessing/check',
				async: true,
				data: {payrefno: payrefno},
				type: "POST",
				dataType: 'json',
				success: function (data) {
					if(check_count >= 300){
	...
	if(typeof(use_main_slider) != "undefined" && use_main_slider === true){
    $('#banner-fade').bjqs({
    	animtype    : 'fade',
      height      : 340,
      width       : 820,
